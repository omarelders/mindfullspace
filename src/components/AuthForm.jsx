...
import { LogIn, UserPlus, AlertCircle, Loader2 } from 'lucide-solid'
import { useAuth } from '../hooks/useAuth'

export function AuthForm(props) {
  // Keep the context object — its getters stay live (no destructuring)
  const auth = useAuth()
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (mode() === 'signup') {
        const { data, error } = await auth.signUp(email().trim(), password(), displayName().trim())
        if (error) {
  ...
  const displayedError = () => localError() || auth.authError

  return (
    <Show
      when={auth.isConfigured}
      fallback={
        <div class="auth-unconfigured-box">
          <AlertCircle class="auth-alert-icon" />
          <div class="auth-unconfigured-text">
    ...
    >
      <div class="auth-form-container">
        ...
        <div class="auth-mode-toggle" role="tablist">
          <button
            type="button"
            role="tab"
            aria-selected={mode() === 'signin'}
            class={`auth-mode-btn ${mode() === 'signin' ? 'is-active' : ''}`}
            onClick={() => {
              setMode('signin')
              setLocalError(null)
              auth.clearError()
          ...
          <button
            ...
            class={`auth-mode-btn ${mode() === 'signup' ? 'is-active' : ''}`}
            onClick={() => {
              setMode('signup')
              setLocalError(null)
              auth.clearError()
        ...
        <Show when={notice()}>
          <div class="auth-notice-banner" role="status">
            <span>{notice()}</span>
          </div>
        </Show>
        ...
        <div class="auth-oauth-row">
          ...
          >
            <svg class="auth-oauth-icon" viewBox="0 0 24 24" width="16" height="16">
              <path
                fill="#4285F4"
          ...
          >
            <svg class="auth-oauth-icon" viewBox="0 0 24 24" width="16" height="16" fill="currentColor">
